...
data. PostGIS uses password authentication; the development password defaults
to `geo-search`. To reuse a volume initialized with a different password, set
`POSTGRES_PASSWORD` to the existing `geo-search` role password in the host
environment or the project-root `.env` before starting the Dev Container. For
passwords containing URL-reserved characters, also set `DATABASE_URL_PASSWORD`
to that password percent-encoded for use in a URL. These settings let the app
connect to the existing role; they do not change the role password or delete
the volume.

To intentionally remove the database and initialize a new empty one, close the
Dev Container and run from a host terminal:
...
To re-import data (e.g., after updates), run the following commands from a
host terminal. `delete-address-data.sh` must run inside the Dev Container, so
it is invoked through `devcontainer exec` (alternatively, run
